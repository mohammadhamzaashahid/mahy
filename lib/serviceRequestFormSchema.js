import { z } from "zod";
import { UAE_DIALING_CODE } from "@/lib/formConstants";
import {
  BUILDING_TYPES,
  BUSINESS_IMPACTS,
  CAR_BRANDS,
...
export const serviceRequestFormSchema = z
  .object({
    ...
    businessImpact: z
      .array(z.string())
      .min(1, "Business Impact required"),
    preferredVisit: requiredSelect(
      PREFERRED_VISITS,
      "Preferred Visit required",
    ),
  ...
  .superRefine((data, ctx) => {
    ...
    if (
      ...
      data.mobileNumber?.length > UAE_MOBILE_DIGITS
    ) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
        message: "UAE mobile number must contain 7 to 9 digits",
        path: ["mobileNumber"],
      });
    }
    ...
    if (isOther || isCar) {
      if (
        ...
        data.problemDescription.trim().length < 1
      ) {
        ctx.addIssue({
          code: z.ZodIssueCode.custom,
